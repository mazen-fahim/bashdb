...
handel_update_query(){
  ...
  #                       -------------
  # 1. will match this -> | update    |
  #                       -------------
  local update_pattern="^update"
 
  #                       --------------------
  # 2. will match this -> |     table_name   |
  #                       --------------------
  local update_table_name_pattern="^\s*($name_pattern)\s*(set)\s*"


  #                       -----------------------------
  # 4. will match this -> | ( c1 = 'value' , c2 = 2 ) |
  #                       -----------------------------
  local column_names_values_pattern="^[[:space:]]*\(([[:space:]]*[a-zA-Z][a-zA-Z0-9_ ]*[[:space:]]*=[[:space:]]*('[^']*'|\d+)([[:space:]]*,[[:space:]]*[a-zA-Z][a-zA-Z0-9_ ]*[[:space:]]*=[[:space:]]*('[^']*'|\d+))*[[:space:]]*)\)$"
  
  local column_names_values_where_pattern="^s*\((\s*$name_pattern=\s*($value_pattern)\s*(,|\)))+where"


  #                       -----------------------------
  # 5. will match this -> | where     <condition>      |
  #                       -----------------------------

  local where_pattern="^\s*([a-zA-Z][a-zA-Z0-9_ ]*)[[:space:]]*(>=|<=|!=|=|>|<)[[:space:]]*('[^']*'|[0-9]+)"
  ...
  query=$(remove_leading_trailing_whitespaces "$query")
  if [[ "$query" =~ $update_table_name_pattern ]]; then
    table_name="$(remove_leading_trailing_whitespaces "${BASH_REMATCH[1]}")"
    query=$(sed -n -r "s/${update_table_name_pattern}//p" <<< "$query")
  else
  ...
  # we used grep here to compare against a query
  # that includes single quotes that make bash complain.
  #f [[ "$found_where" == "false" ]]; then
    query=$(remove_leading_trailing_whitespaces "$query")
    match=$(echo "$query" | grep -P "$column_names_values_pattern")
    echo "match: $match"
    if [[ -n "$match" ]]; then
      matches+=("$(remove_leading_trailing_whitespaces "$match")")
